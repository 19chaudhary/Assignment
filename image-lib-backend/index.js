const express = require('express');
const bodyParser = require('body-parser');
const cors = require('cors');
const app = express();

app.use(bodyParser.json());
app.use(cors());

// Sample data for images
const images = [{
    id: '1',
    url: 'data:image/jpeg;base64,',
    title: 'Beautiful Sunset',
    source: 'Unsplash',
    location: 'Beach',
    isFavorite: false,
  },{
    id: '2',
    url: 'https://cdn.pixabay.com/photo/2014/02/27/16/10/flowers-276014_640.jpg',
    title: 'Green grass',
    source: 'Pixabay',
    location: 'Park',
    isFavorite: false,
  },{
    id: '3',
    url: 'https://webneel.com/daily/sites/default/files/images/daily/08-2018/1-nature-photography-spring-season-mumtazshamsee.jpg',
    title: 'Spring season',
    source: 'Webneel',
    location: 'Mountains',
    isFavorite: false,
  },
  {
    id: '4',
    url: 'https://cdn-ajggd.nitrocdn.com/kMoOFpDlsOVtlYJLrnSRNCQXaUFHZPTY/assets/images/optimized/rev-814242f/wp-content/uploads/bb-plugin/cache/nature-photography-landscape-a892fa3059994f7f5119d4a53fadada3-zybravgx2q47.jpg',
    title: 'Squirrel',
    source: 'Cdn',
    location: 'Tree',
    isFavorite: false,
  },
  {
    id: '5',
    url: 'https://akm-img-a-in.tosshub.com/indiatoday/images/story/201510/thumb_647_102915112526.jpg?VersionId=5DxWM5wXoufOgIeo4TSAFgPvtmZjeV44&size=690:388',
    title: 'A Girl',
    source: 'Akm',
    location: 'Green Nature',
    isFavorite: false,
  },
  // ... other image data
];

// Routes for CRUD operations
app.get('/api/images', (req, res) => {
    console.log(images);
  res.json(images);
});

app.patch('/api/images/:id', (req, res) => {
  const { id } = req.params;
  const { isFavorite, title,source,location } = req.body;

  const imageIndex = images.findIndex((img) => img.id === id);
  if (imageIndex !== -1) {
    if (isFavorite !== undefined) images[imageIndex].isFavorite = isFavorite;
    if (title !== undefined) images[imageIndex].title = title;
    if (source !== undefined) images[imageIndex].source = source;
    if (location !== undefined) images[imageIndex].location = location;

    res.json({ message: 'Image details updated successfully' });
  } else {
    res.status(404).json({ message: 'Image not found' });
  }
});

app.delete('/api/images/:id', (req, res) => {
  const { id } = req.params;
  const index = images.findIndex((img) => img.id === id);

  if (index !== -1) {
    images.splice(index, 1);
    res.json({ message: 'Image deleted successfully' });
  } else {
    res.status(404).json({ message: 'Image not found' });
  }
});

const PORT = process.env.PORT || 5000;
app.listen(PORT, () => {
  console.log(`Server is running on port ${PORT}`);
});
